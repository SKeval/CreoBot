...
function Card({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-cb-card border border-white/[0.08] rounded-2xl p-6 md:p-8">
      {children}
    </div>
  )
}
    ...
    <ul className="flex flex-col gap-2.5">
      {items.map((item) => (
        <li key={item} className="flex items-start gap-3 text-white/60 text-sm leading-relaxed">
          <span className="text-cb-primary-hover mt-0.5 flex-shrink-0">&#10003;</span>
          {item}
        </li>
      ))}
    </ul>
...
export default function TrustPage() {
  return (
    <main className="min-h-screen bg-cb-bg text-white flex flex-col">
      ...
      <section className="flex flex-col items-center justify-center text-center px-6 pt-20 pb-14">
        <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-cb-primary/10 border border-cb-primary/20 mb-6">
          <Shield className="w-7 h-7 text-cb-primary-hover" />
        </div>
      ...
      <div className="max-w-4xl mx-auto w-full px-6 pb-24 flex flex-col gap-8">
        ...
        <Card>
          ...
          <div className="flex flex-col gap-4">
            {[
              ...
              <div key={row.label} className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-4 border-b border-white/[0.08] last:border-0 pb-4 last:pb-0">
                <span className="text-white font-medium text-sm w-40 flex-shrink-0">{row.label}</span>
                <span className="text-white/60 text-sm leading-relaxed">{row.detail}</span>
              </div>
            ))}
          </div>
        </Card>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.08]">
                  <th className="text-left py-3 pr-6 text-white/60 font-medium">Service</th>
                  <th className="text-left py-3 pr-6 text-white/60 font-medium">Purpose</th>
                  <th className="text-left py-3 pr-6 text-white/60 font-medium">Country</th>
                  <th className="text-left py-3 text-white/60 font-medium">Link</th>
                </tr>
              </thead>
              <tbody>
                {subProcessors.map((sp, i) => (
                  <tr key={sp.service} className={i % 2 === 1 ? 'bg-cb-surface/30' : ''}>
                    <td className="py-3.5 pr-6 text-white font-medium">{sp.service}</td>
                    <td className="py-3.5 pr-6 text-white/60">{sp.purpose}</td>
                    <td className="py-3.5 pr-6 text-white/60">{sp.country}</td>
                    <td className="py-3.5">
                      <a
                        href={'https://' + sp.href.replace('https://', '')}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-cb-primary-hover hover:text-cb-primary-hover transition-colors duration-200"
                      >
        ...
        <Card>
          ...
          <div className="bg-cb-primary/5 border border-cb-primary/20 rounded-xl p-4">
            <p className="text-white/70 text-sm leading-relaxed">
              To exercise any of these rights, email{' '}
              <a href="mailto:creobot.alerts@gmail.com" className="text-cb-primary-hover hover:text-cb-primary-hover transition-colors">
        ...
        <Card>
          ...
          <div className="bg-cb-surface rounded-xl p-4 text-sm text-white/70 leading-relaxed">
            Email{' '}
            <a href="mailto:creobot.alerts@gmail.com" className="text-cb-primary-hover hover:text-cb-primary-hover transition-colors">
              creobot.alerts@gmail.com
            </a>{' '}
            with the subject line{' '}
            <span className="text-white font-mono bg-cb-surface px-1.5 py-0.5 rounded text-xs">DPA Request</span>
        ...
        <Card>
          ...
          <div className="flex flex-col gap-3 text-sm">
            <div className="flex items-start gap-4">
              <span className="text-white/40 w-36 flex-shrink-0">Data controller</span>
              <span className="text-white font-medium">CreoBot</span>
            </div>
            <div className="flex items-start gap-4">
              <span className="text-white/40 w-36 flex-shrink-0">Contact email</span>
              <a href="mailto:creobot.alerts@gmail.com" className="text-cb-primary-hover hover:text-cb-primary-hover transition-colors">
                creobot.alerts@gmail.com
              </a>
            </div>
            <div className="flex items-start gap-4">
              <span className="text-white/40 w-36 flex-shrink-0">Last updated</span>
      ...
      <footer className="border-t border-white/[0.08] px-6 pt-8 pb-6">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-white/40 text-sm">&#169; 2026 CreoBot</p>
        </div>
      </footer>
